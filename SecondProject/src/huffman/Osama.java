...
public class Osama {
    ...
    public static byte[][] getHeader(StatisticsTable[] huffmanTable, int numberOfNodes) {
        ...
        // get  huffmanTable length as 4 bit of two node and set into new byte
        byte[] lengths;
        ...
        for (int iIndex = 0, iLoop = 0; iIndex < evenNumber; iLoop += 2) {
            /*
            ...
            if (huffmanTable[iLoop] != null) {
                String s1 = byteToString((byte) huffmanTable[iLoop].getHuffmanLength());
                String s2 = byteToString((byte) huffmanTable[iLoop + 1].getHuffmanLength());

                lengthForTwoByte = s1.substring(4) + s2.substring(4);

                sumOfBitsLength += ((getLengthOfHuffmanCode(lengthForTwoByte, true)) +
                        (getLengthOfHuffmanCode(lengthForTwoByte, false)));

                lengths[iIndex++] = Byte.parseByte(lengthForTwoByte, 2);
            }

        }
        if (!isEven) {
            String s = byteToString((byte) huffmanTable[numberOfNodes - 1].getHuffmanLength());
            lengthForTwoByte = s.substring(4) + "0".repeat(4);
            lengths[evenNumber] = Byte.parseByte(lengthForTwoByte, 2);
        }
    ...
    private static byte[] getTwoByteFromSting(String huffmanCode) {
        byte[] twoByte = new byte[2];
        twoByte[0] = Byte.parseByte(huffmanCode.substring(0, 8), 2);
        twoByte[1] = Byte.parseByte(huffmanCode.substring(8), 2);
        return twoByte;
    }
